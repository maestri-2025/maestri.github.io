...
for a in artists:
    ...
    nodes = {}
    links = []
    nodes[artist_id] = {"id":str(artist_id),
                        "name": a["name"],
                        "height":1,
                        "size":24,
                        "color": "rgb(97, 205, 187)"
    ...
    for c in a["contributions"]:
        ...
        try:
            if c["primary_artist_id"] in nodes:
                nodes[c["primary_artist_id"]]["height"] += 1 
            else:
                nodes[c["primary_artist_id"]] =  {
                                                "id": str(c["primary_artist_id"]),
                                                "name": "",
                                                "height": 1,
                                                "size": 24,
                                                "color": "rgb(97, 205, 187)"
    ...
    for c in a["contributors"]:
        ...
        if c["contributor_artist_id"] in nodes:
            nodes[c["contributor_artist_id"]]["height"] += 1
        else:
            nodes[c["contributor_artist_id"]] =  {
                                            "id": str(c["contributor_artist_id"]),
    ...
    artist_graphs[artist_id] = {"nodes":list(nodes.values()), "links":links}
